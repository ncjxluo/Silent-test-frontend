import {
  ...
  get_message_channel,
  setting_message_status,
  del_message_channel,
  get_message_channel_status,
} from '@/api/message.ts'
...
export const message_channel = () => {
  ...
  const get_messages_status = async (query: any) => {
    const res = await get_message_channel_status(query)
    return res.data.data
  }
  ...
  return { test_send, save_message, get_messages, set_message_status, del_message, get_messages_status }
}
